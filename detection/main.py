...
# detection codebase specific imports
...
# When accessing the working directory later:
try:
    os.getcwd()
except FileNotFoundError:
    os.chdir(initial_working_dir)  # Recover to initial path
...
def make_args_parser():
    ...
    parser.add_argument('--optimizer_part', type=str, default='adapt',
    ...
    parser.add_argument('--rank', type=int, default=36, help='rank of adapter')
    parser.add_argument('--drop_adapt', type=float, default=0., help='adapter drop_path')
    parser.add_argument('--drop_path', type=float, default=0., help='adapter drop_path')
...
def do_train(
    ...
    if not args.load_from_checkpoint:
        timestr = str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M_%S_'))
        timestr = timestr + args.model_name
...
def main(local_rank, args):
    ...
    if args.test_only:
        ...
        if is_primary() and not os.path.isdir(args.checkpoint_dir) and not args.load_from_checkpoint:
        ...
        if args.load_from_checkpoint:
            ...
            best_val_metrics=ckpt['best_val_metrics']
