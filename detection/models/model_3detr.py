...
from models.Point_BERT import PointTransformerBERT_PGST
from models.Point_BERTraw import PointTransformerBERTraw


class BoxProcessor(object):
    """
    Class to convert detection MLP head outputs into bounding boxes
    """
...
class Model3DETR(nn.Module):
    """
    Main detection model. Consists of the following learnable sub-models
    ...
    def run_encoder(self, point_clouds):
        ...
        #     pre_enc_features, pre_enc_xyz = self.backbone(pre_enc_features, pre_enc_xyz)

        # # xyz points are in batch x npointx channel order
        # enc_xyz, enc_features, enc_inds = self.encoder(
        #     pre_enc_features, xyz=pre_enc_xyz
        # )
        enc_features, enc_xyz = self.encoder(pre_enc_features, pre_enc_xyz)
        enc_inds=None
        if enc_inds is None:
...
def build_3detr(args, dataset_config):
    ...
    if args.model_name=='3detr':
        ...
        if 'gft' in args.model_name:
            encoder = PointTransformerBERT_PGST(args)
            encoder.load_model_from_ckpt(name=args.model_name)
        else:
            encoder = PointTransformerBERTraw(args)
            encoder.load_model_from_ckpt(name=args.model_name)
        model = Model3DETR(
            pre_encoder,
            encoder,
